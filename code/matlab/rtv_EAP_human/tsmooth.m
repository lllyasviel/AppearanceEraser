function S = tsmooth(I,mask,lambda,sigma,sharpness,maxIter)
...
%
    ...
    for k=1:piter
        I = I.*Mw;
        I = I(:, [2:width, width], :) + I([2:height, height], :, :) + I(:, [1, 1:width-1], :) + I([1, 1:height-1], :, :);
        Mw = Mw(:, [2:width, width], :) + Mw([2:height, height], :, :) + Mw(:, [1, 1:width-1], :) + Mw([1, 1:height-1], :, :);
        I = I./Mw;
        Mw = Mw/4.0;
        Mw(Mw<0.25)=0;
        Mw(Mw>0)=1;
